...
<body>
	...
	<%
		...
		if (mahoadon != null) {
			...
			HoaDonTamDAO daohdt = new HoaDonTamDAO();
			hdt = daohdt.getHoaDonTam(mahoadon);
			if (hdt.getNguoiLap() != null) {
				response.sendRedirect("./gdHoaDonTam.jsp");
				session.setAttribute("nhanvien", nv);
				session.setAttribute("hoadontam", hdt);
			} else {
	%>
	...
	<div class="search">
		<div class="container">
			<div class="search-iner">
				<form action="gdTimHoaDonTam.jsp">
					<div class="mb-3 mt-3">
						<label for="text" class="form-label">Tìm kiếm hóa đơn tạm:</label>
						<div class="input">
							<input type="text" class="form-control" id="text"
								placeholder="Nhập mã hóa đơn tạm" name="mahoadon"> 
								<a><button type="submit" class="btn btn-primary tim">Tìm
									kiếm</button></a>
						</div>
					</div>

				</form>
				<a href="./gdTrangChu.jsp">
					<button type="submit" class="btn btn-primary ql">Quay lại</button>
				</a>
			</div>
		</div>
	</div>
